      ...
      if($this->Request->saveAll($this->request->data)){ 
        ...
        if($this->Subscriber->saveField('user_id', $user["User"]["id"])){
          ...
          if ($this->Session->read('Auth.User')){
            ...
            $this->Session->setFlash('<h4>Your request has been submitted!</h4><p class="lead">You can expect a response from the  '. $this->getAgencyName() .'  in the next ' . $this->getResponseDays() . ' business days. If you have provided an email, you will be contacted via email with any updates.</p>', 'success');
          }
